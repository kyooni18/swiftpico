...
extension SwiftPicoCommand {
    ...
    static func doctor(_ arguments: [String]) throws {
        ...
        let picoKitRoot = project.flatMap { try? resolvePicoKitRoot(project: $0, config: $0.config) } ?? findPicoKitRoot(from: current)
        ...
        if let project {
            ...
            print("  Lock file:   \(FileManager.default.fileExists(atPath: lock.path) ? "available" : "MISSING (run swiftpico dependencies resolve)")")
            print("  CMake state: \(FileManager.default.fileExists(atPath: generated.path) ? "generated" : "MISSING (run swiftpico dependencies generate)")")
            print("  Build state: \(FileManager.default.fileExists(atPath: state.path) ? "recorded" : "not built")")
        } else {
