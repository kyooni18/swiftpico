...
extension SwiftPicoCommand {
    ...
    private static func monitorImpl(_ arguments: [String]) throws {
        ...
        var device = try SerialMonitorConfiguration.selectDevice(explicit: explicitDevice, detected: detectedDevices)
        ...
        interrupt.setEventHandler {
            restoreTerminal?()
            writeMonitorStatus("Serial monitor stopped. Sent \(traffic.sent) bytes; received \(traffic.received) bytes.")
            Foundation.exit(0)
        }
        interrupt.resume()
            ...
            guard let data = connection.read() else {
                ...
                while true {
                    ...
                    if let explicitDevice {
                        ...
                        guard let detected = SerialMonitorConfiguration.reconnectCandidate(
